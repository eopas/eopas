...
namespace :delayed_job do
 ...
 task :stop, :roles => :app do
   run "cd #{current_path} && RAILS_ENV=#{environment} script/delayed_job stop"
 end
 desc "Start the delayed_job process"
 task :start, :roles => :app do
   run "cd #{current_path} && RAILS_ENV=#{environment} script/delayed_job start"
 end
 desc "Restart the delayed_job process"
 task :restart, :roles => :app do
   run "cd #{current_path} && RAILS_ENV=#{environment} script/delayed_job restart"
 end
end